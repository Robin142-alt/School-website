...
function SectionCard({
  ...
  return (
    <section className="rounded-[2rem] border border-brand-forest/10 bg-white/84 p-6 shadow-lg shadow-brand-forest/5 sm:p-7">
      <div className="flex items-start gap-4">
        <div className="rounded-2xl bg-brand-sage/35 p-3 text-brand-forest">{icon}</div>
  ...
  if (!configured) {
    return (
      <section className="section-shell py-12 sm:py-16">
        <div className="mx-auto max-w-3xl rounded-[2rem] border border-brand-forest/10 bg-white/84 p-8 text-center shadow-lg shadow-brand-forest/5">
          <p className="text-sm font-semibold uppercase tracking-[0.22em] text-brand-forest">
            Staff portal
          </p>
          ...
          <p className="mt-4 text-base leading-8 text-muted">
            Add <code className="rounded bg-brand-sage/35 px-2 py-1 text-sm">ADMIN_ACCESS_KEY</code>
      ...
      <section className="section-shell py-12 sm:py-16">
        <div className="mx-auto grid max-w-5xl gap-8 lg:grid-cols-[1fr_0.9fr]">
          <div className="space-y-6">
            <span className="inline-flex rounded-full border border-brand-gold/30 bg-brand-gold/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-brand-forest">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="rounded-[1.7rem] border border-brand-forest/10 bg-white/78 p-5">
                <Newspaper className="h-5 w-5 text-brand-forest" />
              ...
              <div className="rounded-[1.7rem] border border-brand-forest/10 bg-white/78 p-5">
                <LineChart className="h-5 w-5 text-brand-forest" />
                <p className="mt-3 text-sm leading-7 text-muted">Refresh trust signals such as KCSE performance.</p>
              </div>
            </div>
          </div>

          <div className="rounded-[2rem] border border-brand-forest/10 bg-white/84 p-6 shadow-lg shadow-brand-forest/5 sm:p-8">
            <p className="text-sm font-semibold uppercase tracking-[0.22em] text-brand-forest">
              Sign in
            </p>
    ...
    <section className="section-shell py-10 sm:py-14">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        ...
        <form action={logoutPortalAction}>
          <button
            type="submit"
            className="inline-flex items-center justify-center rounded-full border border-brand-forest/15 bg-white/80 px-5 py-3 text-sm font-semibold text-brand-forest"
          >
            Sign out
          </button>
        </form>
      ...
      <div className="mt-10 grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        ...
        >
          <div className="grid gap-4">
            {inquiries.length === 0 ? (
              <p className="rounded-[1.5rem] bg-brand-sage/20 px-4 py-4 text-sm text-muted">
              ...
              inquiries.map((inquiry) => (
                <article
                  key={inquiry.id}
                  className="rounded-[1.5rem] border border-brand-forest/10 bg-brand-sage/18 p-4"
                >
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <p className="text-xs uppercase tracking-[0.18em] text-brand-forest">
              ...
              <div>
                ...
                <div className="mt-3 grid gap-3">
                  {newsPosts.slice(0, 4).map((item) => (
                    <div key={item.slug} className="rounded-[1.4rem] bg-brand-sage/18 px-4 py-4">
              ...
              <div>
                ...
                <div className="mt-3 grid gap-3">
                  {events.slice(0, 4).map((item) => (
                    <div key={`${item.title}-${item.startDate}`} className="rounded-[1.4rem] bg-brand-sage/18 px-4 py-4">
                      <p className="text-sm font-semibold text-brand-ink">{item.title}</p>
                      <p className="mt-1 text-xs uppercase tracking-[0.18em] text-muted">
              ...
              <div>
                ...
                <div className="mt-3 grid gap-3">
                  {performanceData.series.map((item) => (
                    <div key={item.year} className="rounded-[1.4rem] bg-brand-sage/18 px-4 py-4">
